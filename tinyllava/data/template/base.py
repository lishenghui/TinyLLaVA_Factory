...
def add_integer_to_2d_list(lst, integer):
    return [
        [(element + integer) if element != -1 else -1 for element in sublist]
        for sublist in lst
    ]
...
class Template:
    ...
    def _concat_two_intervention_locations(
        self, intervention_locations, intervention_locations_2
    ) -> Tuple[List[List], int, int]:
        assert len(intervention_locations) == len(
            intervention_locations_2
        ), "The length of two intervention_locations doesn't equal, both should be the number of interventions."
        return (
            [
                intervention_locations[i] + intervention_locations_2[i]
                for i in range(len(intervention_locations))
            ],
            len(intervention_locations[0]),
            len(intervention_locations_2[0]),
        )

    def get_intervention_locations_for_multimodal_prompt(
        self, question_prompt, tokenizer, question_content
    ):
        if isinstance(self.mores_pos_configs, Dict):
            return self._get_intervention_locations_for_multimodal_prompt(
                question_prompt, tokenizer, question_content, self.mores_pos_configs
            )
        elif isinstance(self.mores_pos_configs, List) and isinstance(
            self.mores_pos_configs[0], Dict
        ):
            intervention_locations = (
                self._get_intervention_locations_for_multimodal_prompt(
                    question_prompt,
                    tokenizer,
                    question_content,
                    self.mores_pos_configs[0],
                )
            )
            intervention_locations_2 = (
                self._get_intervention_locations_for_multimodal_prompt(
                    ...
                    tokenizer,
                    question_content,
                    self.mores_pos_configs[1],
                )
            )
            return self._concat_two_intervention_locations(
                intervention_locations, intervention_locations_2
            )
        else:
            raise ValueError("invalid format of mores_pos_configs in Template")

    def _get_intervention_locations_for_multimodal_prompt(
        self, question_prompt, tokenizer, question_content, mores_pos_configs
    ):
        # intervention locations
        question_prompt_ids = self.tokenizer_image_token(
            question_prompt, tokenizer, return_tensors="pt"
        )
        ...
        question_content_ids = self.tokenizer_image_token(
            question_content, tokenizer, return_tensors="pt"
        )
        question_content_length = len(question_content_ids)
        sys_length = question_prompt_length - question_content_length

        if bool(
            re.fullmatch(r"^f\d+\++l\d+$", mores_pos_configs["intervention_positions"])
        ):
            intervention_locations, start_pos, end_pos, intervention_part_len = (
                self._intervention_locations_fl(
                    sys_length, question_prompt_length, num_img_token, mores_pos_configs
                )
            )
        elif bool(
            re.fullmatch(r"^uniform\d+$", mores_pos_configs["intervention_positions"])
        ):
            intervention_locations, start_pos, end_pos, intervention_part_len = (
                self._intervention_locations_uniform(
                    sys_length, question_prompt_length, num_img_token, mores_pos_configs
                )
            )
        else:
            raise ValueError(
                'Get invalid intervention_positions! It should be like either "f4+l5" or "uniform9"!'
            )

        intervention_locations = add_integer_to_2d_list(
            intervention_locations, start_pos
        )
        if (
            all(x == -1 for x in intervention_locations[0])
            and intervention_part_len == 1
        ):
            if "text" in mores_pos_configs["intervene_modality"]:  # condition 1
                intervention_locations = [
                    [start_pos] + [-1] * (len(intervention_locations[0]) - 1)
                ] * len(intervention_locations)
            else:  # condition 2
                intervention_locations = [
                    [start_pos - 1] + [-1] * (len(intervention_locations[0]) - 1)
                ] * len(intervention_locations)
                assert (
                    start_pos - 1 >= 0
                ), "There's no (sys) token in front of the text tokens!"
        replace_pad_in_2d_list_intervention_locations(intervention_locations)
        return intervention_locations

    def _intervention_locations_fl(
        self, sys_length, question_prompt_length, num_img_token, mores_pos_configs
    ):
        ...
        assert (
            "text" in mores_pos_configs["intervene_modality"]
            or "vis" in mores_pos_configs["intervene_modality"]
        ), 'intervene_modality has to at least include one of "vis" and "text"'
        if "text" in mores_pos_configs["intervene_modality"]:
            end_pos = (
                question_prompt_length - 1 + (img_embed_token_len - 1) * num_img_token
            )
        else:
            end_pos = (
                sys_length - 1 + img_embed_token_len * 1
                if num_img_token != 0
                else sys_length
            )
        if "vis" in mores_pos_configs["intervene_modality"]:
            ...
            start_pos = (
                sys_length + (img_embed_token_len) * 1
                if num_img_token != 0
                else sys_length
            )
        ...
        assert (
            intervention_part_len >= 1
        ), "There's no question content (zero tokens) to interevene!"
    ...
    def _intervention_locations_uniform(
        self, sys_length, question_prompt_length, num_img_token, mores_pos_configs
    ):
        num_uni_samp = parse_positions_uniform(
            mores_pos_configs["intervention_positions"]
        )
        img_embed_token_len = mores_pos_configs["img_embed_token_len"]

        assert (
            "vis" in mores_pos_configs["intervene_modality"]
        ), 'For uniform sampled positions, intervene_modality has to be "vis", which has the fixed length of token'
    ...
    def encode(self, messages, tokenizer, mode="train", return_sys_len=False):
        ...
        prompt, question_prompt, question_content = self.prompt(
            question_list, answer_list, return_prompt=True
        )
        input_ids = self.tokenizer_image_token(prompt, tokenizer, return_tensors="pt")
        sys_len = (
            self.get_sys_len(question_prompt, tokenizer, question_content)
            if return_sys_len
            else None
        )
        ...
        if self.mores_pos_configs != None:
            if isinstance(self.mores_pos_configs, Dict):
                intervention_locations = (
                    self.get_intervention_locations_for_multimodal_prompt(
                        question_prompt, tokenizer, question_content
                    )
                )
            elif isinstance(self.mores_pos_configs, List) and isinstance(
                self.mores_pos_configs[0], Dict
            ):
                intervention_locations, len_pos_1, len_pos_2 = (
                    self.get_intervention_locations_for_multimodal_prompt(
                        question_prompt, tokenizer, question_content
                    )
                )
    ...
    def _get_list_from_message(self, messages, mode="train"):
        ...
        if mode == "train":
            assert len(question_list) == len(
                answer_list
            ), f"qa is not match : length_q:{len(question_list)} vs length_a:{len(answer_list)}"
        return question_list, answer_list

    def prompt(self, question_list, answer_list, return_prompt: bool = False):
        if type(question_list) is str:
            question_list = [question_list]
        if type(answer_list) is str:
            answer_list = [answer_list]
        msg, question_prompt, question_content = (
            self._prompt(question_list, answer_list)
            if len(answer_list) == len(question_list)
            else self._prompt_only_q(question_list)
        )
    ...
    def _prompt_only_q(self, question_list):
        msg = ""
        question_prompt = ""
        question_content = None
    ...
    def make_labels(self, input_ids, prompt, tokenizer):
        ...
        if cur_len < tokenizer.model_max_length:
            ...
            if cur_len != total_len:
                raise ValueError(f"tokenization mismatch: {cur_len} vs. {total_len}.")
                print(
